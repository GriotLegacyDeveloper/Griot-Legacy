...
import 'package:griot_legacy_social_media_app/src/models/get_advertisement_response_model.dart';
...
class AdvertisementEditPage extends StatefulWidget {
  final ResponseData advertisementData;
  final AdvertisementEditPageInterface mListener;
  const AdvertisementEditPage({Key key, this.advertisementData,this.mListener}) : super(key: key);

  @override
  _AdvertisementEditPageState createState() => _AdvertisementEditPageState();
}

class _AdvertisementEditPageState extends State<AdvertisementEditPage> {
  ...
  void initState() {
    ...
    super.initState();
    if (widget.advertisementData != null) {
      companyName.text = widget.advertisementData.companyName;
      contactPerson.text = widget.advertisementData.contactPerson;
      email.text = widget.advertisementData.emailAddress;
      address.text = widget.advertisementData.physicalAddress;
      phone.text = "${widget.advertisementData.phoneNumber}";
      purposeOdAdv.text = widget.advertisementData.purposeOfAdvertisement;
      discription.text = widget.advertisementData.description;
      link.text = widget.advertisementData.link;
      title.text = widget.advertisementData.title;
      validFromController.text =DateFormat('MM/dd/yyyy').format(DateTime.parse(widget.advertisementData.validFrom));
      startDate = DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.advertisementData.validFrom));
      validTillController.text =DateFormat('MM/dd/yyyy').format(DateTime.parse(widget.advertisementData.validTill));
      endDate =DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.advertisementData.validTill));
      countryCode.text=widget.advertisementData.countryCode;
    }
  }
  ...
  Widget pageBuilder(UserController con) {
    ...
    if (buildFlag) {
      print("1111countru");
   //   con.nationCode = "${widget.advertisementData.countryCode}";
      buildFlag = false;
    }
                      ...
                      children: [
                        Expanded(
                          child: MyCountryCodePicker(
                              dialogBackgroundColor: hp.theme.secondaryHeaderColor,
                              ...
                              backgroundColor: hp.theme.cardColor, barrierColor: hp.theme.primaryColorLight,
                              textStyle: TextStyle(color: hp.theme.secondaryHeaderColor),
                              tc: countryCode),
                        ),
                        Expanded(
                          flex: 2,
...
//keyboardType: TextInputType.name,
                        ...
                        _image == null
                         ?Container(
                          ...
                          child: Padding(
                            ...
                            child: Image.network(widget.advertisementData.image,
                              fit: BoxFit.cover,
                            ),
                          ),
                        )
  ...
  GetAdvertisementResponsemodel editAdsModel = GetAdvertisementResponsemodel();
 ...
 editAdsApi() async {
   ...
   print("11111...22222222222222..${_image!=null}");

    controller.waitUntilEditAdvertisement(
        _image!=null ?_image.path : _image,
        companyName.text,
        contactPerson.text,
        email.text,
        countryCode.text,
      ...
      editAdsModel = controller.editAdsResponse;

      print("55555555");

      if (editAdsModel.responseData.isNotEmpty) {
        widget.mListener.addPaymentScreen(
            editAdsModel.responseData.elementAt(0).sId,
            editAdsModel.responseData.elementAt(0).title);
      }
      else{
...
abstract class AdvertisementEditPageInterface {
